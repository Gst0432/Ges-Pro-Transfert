            ...
            <div className="grid grid-cols-3 gap-4">
                <div className="col-span-2">
                    ...
                    <Select onValueChange={setSelectedProduct} value={selectedProduct}>
                        ...
                        <SelectContent>
                            {products.map(p => <SelectItem key={p.id} value={p.id}>{p.name} ({p.price} FCFA)</SelectItem>)}
                        </SelectContent>
                    </Select>
                </div>
                <div>
...
const Step3Finalization = ({ onBack, onSave, saleData, setSaleData }) => {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex justify-between mt-8">
                <Button variant="outline" onClick={onBack}>&larr; Précédent</Button>
                <Button onClick={onSave}>Enregistrer la vente</Button>
            </div>
        </div>
    );
};
...
export const NewSaleWizard = ({ isOpen, onOpenChange, onSaleSaved }) => {
  ...
  const renderStep = () => {
    switch (currentStep) {
      ...
      case 3:
        return <Step3Finalization onBack={handleBack} onSave={handleSave} saleData={saleData} setSaleData={setSaleData} />;
      default:
        return null;
    }
  };
